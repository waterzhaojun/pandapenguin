function [result, array_sec] = get_bout_drewlab(array, scanrate, varargin)
parser = inputParser;
...
config = run_config();
array_treat = lowpass(array, 1, scanrate);
array_treat = abs(diff([0, array_treat]));
array_treat = heaviside(array_treat - config.acceleration_threshold);
