...
%get the global info variable
    ...
    [folderpath, filepath, extension] = fileparts(path);
    filepath = strsplit(filepath,'_');
    filepath = [filepath{1},'_',filepath{2},'_',filepath{3}];
%     if strcmp(path(end - 3:end), '.sbx')
%         path = [path(1:end - 4) '.mat'];
%     elseif ~strcmp(path(end - 3:end), '.mat')
%         path = [path '.mat'];
%     end
    path = [folderpath, '\', filepath, '.mat']; 
    base = [folderpath, '\', filepath];
%     base = path(1:end - 4);
